"""
HostLLMProxy — LLM access without direct API keys
===================================================
...
class HostLLMProxy(BaseChatModel):
    ...
    mode: str = "fake"  # fake | ollama | mcp | proxy
    ...
    def _generate(
        ...
        **kwargs: Any,
        ...
        if self.mode == "fake":
            ...
            # 이 모드는 프레임워크가 MCP 도구로 실행될 때, 
            # 호스트 AI에게 "네가 이 답변을 생성해줘"라고 요청하는 마커를 반환합니다.
            response_text = f"[HOST_AI_REQUIRED: Please process this request based on the following context context: {last_msg}]"
        else:
            response_text = f"Mode '{self.mode}' is not yet implemented. Using fake response: Hello! I am Langent v2."
    ...
    def _fake_response(self, prompt: str) -> str:
        ...
        if "안녕" in p_lower or "hello" in p_lower:
            return "안녕하세요! Langent v2 프레임워크의 에이전트입니다. 무엇을 도와드릴까요?"
        if "치킨" in p_lower:
            return "서울에 위치한 60계치킨, 처갓집양념치킨 등 다양한 치킨집 정보가 벡터 DB에 저장되어 있습니다. 특정 지역을 말씀하시면 상세히 찾아드릴게요."
        if "cypher" in p_lower or "match" in p_lower:
            return "MATCH (n:Person) RETURN n LIMIT 5"
        
        return f"Langent 프레임워크가 '{prompt}'에 대한 처리를 준비 중입니다. 현재는 테스트 모드(fake)입니다."
